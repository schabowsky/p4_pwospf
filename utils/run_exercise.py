...
class ExerciseRunner:
    ...
    def program_switch_p4runtime(self, sw_name, sw_dict):
        ...
        if "interfaces" in sw_dict:
            for iface, if_dict in sw_dict["interfaces"].iteritems():
                if_name = sw_name + "-" + iface
                print if_dict["ip"], if_dict["mac"], if_name
                sw_obj.setIP(if_dict["ip"], intf=if_name)
                sw_obj.setMAC(if_dict["mac"], intf=if_name)
    ...
    def add_controllers(self):
        for sw_name, sw_dict in self.switches.iteritems():
            ctrl_name = sw_name.replace("s", "c")
            ip_addr = "10.0.0." + ctrl_name[-1]
            print ip_addr
            ctrl_obj = self.net.addController(ctrl_name, ip=ip_addr)
            self.net.addLink(sw_name, ctrl_name)
